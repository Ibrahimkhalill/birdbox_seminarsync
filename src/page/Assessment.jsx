...
import Assessmendivetail from './AssessmentDetail.JSX';
...
const Assessment = ({ item }) => {
	...
	const handleSearch = (e) => {
		...
		const filteredParticipants = originalParticipants.filter((participant) =>
						...
						{participants.map((participant, index) => (
							<li
								...
								className={`py-2 px-2 text-center border-b border-gray-800 cursor-pointer ${
									participant.first_name === selectedParticipant?.first_name &&
									participant.last_name === selectedParticipant?.last_name
										? 'bg-[#092147]'
