...
const Sidebar = ({ children, title }) => {
	...
	return (
		<div className="flex flex-col min-h-screen">
			<nav className="  fixed top-0 z-50 w-full bg-[#171717] shadow">
							...
							<div className="flex items-center ms-3 gap-2">
								<div className="relative">
									...
									{isDropdownOpen && (
										<div
											ref={dropdownRef}
											className="w-[calc(100vw-48px)] sm:!w-[343px] fixed sm:!absolute bg-black top-[55px] right-0 smx:-right-4 left-4 sm:left-[-308px] rounded-lg overflow-hidden z-[15] shadow-[rgba(50,50,93,0.25)_0px_13px_27px_-5px,rgba(0,0,0,0.3)0px_8px_16px_-8px]">
											<div className="w-full p-2 flex flex-col justify-center items-center gap-2">
												<div className="p-2 flex items-center gap-2 self-stretch rounded-lg  shadow-[0px_1px_1px_0px_rgba(0,0,0,0.12)]">
													<div className="rounded-full w-12 h-12 relative overflow-hidden">
														<div className="rounded-full aspect-square object-cover w-full border flex items-center justify-center text-xl uppercase text-t_color bg-blue-600">
															{email[0]}
														</div>
													</div>
													<div className="flex flex-col justify-center items-start gap-0.5 flex-1">
														...
														<p className="text-body-b2 text-t_color">{email}</p>
													</div>
												</div>
			...
			<aside
				...
				} bg-[#121212]  sm:translate-x-0`}
				aria-label="Sidebar">
				<div className="h-full px-3 pb-4 overflow-y-auto bg-[#121212] ">
					<ul className="space-y-2 font-medium mt-10">
						<li>
							<Link
								...
								/>
								<span className="ms-3 text-t_color">Home</span>
							</Link>
						</li>
							...
							<li>
								<Link
									...
									<LuBookAudio
										...
										className="transition duration-75 group-hover:text-t_color"
									/>
									<span className="ms-3 text-t_color">Coach list</span>
								</Link>
							</li>
						)}
						...
						<li className="absolute bottom-10 w-[85%]">
							<button
								onClick={() => setIsVisble(!isVisible)}
								className="flex items-center p-2 text-t_color rounded-lg hover:bg-blue-900 group w-full">
								<MdLogout
									size={25}
									className="transition duration-75 group-hover:text-t_color"
								/>
			...
			<div className="flex-1  bg-[#1C1C1D] h-auto sm:ml-60 mt-14 overflow-auto py-10  ">
				{children}
			</div>
			{role === 'admin' ? (
				<AdminProfile setIsOpen={setIsOpen} isOpen={isOpen} />
			) : (
				<EditProfileModal setIsOpen={setIsOpen} isOpen={isOpen} />
			)}
		</div>
	);
};
