...
const EditProfileModal = ({ isOpen, setIsOpen }) => {
		...
		if (isOpen) {
			const fetchProfile = async () => {
				...
				try {
					...
					setImageName(data.image ? data.image.split('/').pop() : ''); // Extract filename from URL
				} catch (err) {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			data.append('working_experience', formData.working_experience);
			...
			const response = await axiosInstance.put('/accounts/update/', data, {
				headers: {
				...
				<div
					...
					<div className="bg-[#0F0F0F] p-6 rounded-lg shadow-lg w-[30rem]">
						<div className="flex justify-between items-center mb-4">
							...
							<h2 className="text-lg font-bold text-t_color">Edit Profile</h2>
							<span></span>
						</div>
									...
									<div className="flex items-center">
										...
										{imageName && (
											<div className="ml-3 flex items-center">
												<span className="text-t_color">{imageName}</span>
												<button
													type="button"
